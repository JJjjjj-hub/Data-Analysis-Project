...
## 1. 背景与目标
- 交付一个可通过 Web 页面操作的通用数据分析系统，覆盖完整链路：注册/登录 → 上传 → 清洗 → 训练 → 预测 → 可视化 → 导出。
- 支持任意 CSV 数据的二分类任务，用户可在训练时指定目标列。
...
- 典型演示路径
  ...
  2. 上传任意 CSV，自动预览前 20 行与列类型
  3. 执行清洗，查看清洗摘要
  4. 选择目标列并训练模型，展示指标与混淆矩阵
  5. 调整阈值做单条预测预览
  6. 查看柱状统计、箱线分布、散点相关图
  7. 导出清洗后 CSV
